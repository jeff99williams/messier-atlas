...
Constellation lines and boundaries are loaded on demand from the version-pinned
[d3-celestial](https://github.com/ofrohn/d3-celestial) GeoJSON data package.

Present-day estimate arrows are modelled only for galaxies outside the Local
Group, using a simple 70 km/s/Mpc Hubble-flow approximation. Local objects need
proper-motion and velocity vectors that are not part of this catalogue.

City lookup for the sky chart uses the no-key
